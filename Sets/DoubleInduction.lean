...
-- For a function g, (gx_sub_y_or_y_sub_x g) is the relation {(x, y) | g⟨x⟩ ⊆ y ∨ y ⊆ x}.
protected def gx_sub_y_or_y_sub_x_P₂ (g) [IsRelation g] [IsFunction g] := P₂ (fun p => ∃ (x y : Class) (_ : IsSet x) (_ : IsSet y) (_ : InDom x g), p = OrdPair x y ∧ (g⟨x⟩ ⊆ y ∨ y ⊆ x))
protected noncomputable def gx_sub_y_or_y_sub_x (g) [IsRelation g] [IsFunction g] : Class := (DoubleInduction.gx_sub_y_or_y_sub_x_P₂ g).choose
...
protected theorem gx_sub_y_or_y_sub_x_φ_pair (g x y) [IsRelation g] [IsFunction g] [IsSet x] [IsSet y] [InDom x g] : OrdPair x y ∈ DoubleInduction.gx_sub_y_or_y_sub_x g ↔ g⟨x⟩ ⊆ y ∨ y ⊆ x :=
  have mp : OrdPair x y ∈ DoubleInduction.gx_sub_y_or_y_sub_x g → g⟨x⟩ ⊆ y ∨ y ⊆ x :=
    fun h =>
  ...
  have mpr : g⟨x⟩ ⊆ y ∨ y ⊆ x → OrdPair x y ∈ DoubleInduction.gx_sub_y_or_y_sub_x g :=
    fun h =>
...
protected instance (g) [IsRelation g] [IsFunction g] : IsRelation (DoubleInduction.gx_sub_y_or_y_sub_x g) where
  prop :=
    let r := DoubleInduction.gx_sub_y_or_y_sub_x g
    fun (p) (p_in_r : p ∈ r) =>
...
theorem ProgressingFunctionLemma_1 (g) [IsRelation g] [IsFunction g] : ∀ y [IsSet y] [InDom y g], (OrdPair y Null) ∈ DoubleInduction.gx_sub_y_or_y_sub_x g :=
...
theorem ProgressingFunctionLemma_2 (g) [IsRelation g] [IsFunction g] : ∀ x y [IsSet x] [IsSet y] [InDom x g] [InDom y g], is_progressing g → (OrdPair x y) ∈ DoubleInduction.gx_sub_y_or_y_sub_x g → (OrdPair y x) ∈ DoubleInduction.gx_sub_y_or_y_sub_x g → (OrdPair x g⟨y⟩) ∈ DoubleInduction.gx_sub_y_or_y_sub_x g :=
...
theorem P₂_PFL_R_is_double_ind_relation (m g : Class) [IsRelation g] [IsFunction g] (m_sub_dom : m ⊆ Dom g) (g_prog : is_progressing g) : DoubleInduction.is_double_ind_relation m (DoubleInduction.gx_sub_y_or_y_sub_x g) g:=
  let r := DoubleInduction.gx_sub_y_or_y_sub_x g
...
theorem T_4_4_7 (M g) [IsRelation g] [IsFunction g] : M ⊆ Dom g → is_min_inductive_under M g → is_progressing g →
  (is_nest M ∧ ∀ x y [IsSet x] [IsSet y] [InDom x g], x ∈ M → y ∈ M → (OrdPair x y) ∈ DoubleInduction.gx_sub_y_or_y_sub_x g) :=
  ...
  have in_r : ∀ x y [IsSet x] [IsSet y] [InDom x g], x ∈ M → y ∈ M → (OrdPair x y) ∈ r :=
    fun x y _ _ _ x_in_m y_in_m =>
    DIP M r g hm r_is_double_ind_r x y x_in_m y_in_m
  have m_is_nest : is_nest M :=
    fun (x y) (_ : IsSet x) (_ : IsSet y) (x_in_m) (y_in_m) =>
    haveI : InDom x g := ⟨ m_sub_domg x x_in_m ⟩
    have pair_in_r : (OrdPair x y) ∈ r := in_r x y x_in_m y_in_m
    let ⟨ a, b, _, _, _, prop ⟩ := (DoubleInduction.gx_sub_y_or_y_sub_x_φ g (OrdPair x y)).mp pair_in_r
    have op_eq_op : OrdPair x y = OrdPair a b := prop.left
    have term_eq : x = a ∧ y = b := ord_pair_ident op_eq_op
    have t1 : g⟨x⟩ ⊆ y ∨ y ⊆ x := term_eq.right ▸ (term_eq.left ▸ prop.right)
    Or.elim t1
      (fun gx_sub_y =>
